...
def all_persistent_response_onsets(
    ...
    # --------------------------------------------------
    # Strong-response filter
    # --------------------------------------------------
    # Normalized response strength:
    #
    # max(
    #   relative_rigid / relative_rigid_thr,
    #   relative_q75  / relative_point_thr
    # )
    #
    # Require >= 1.5x AND original HYBRID-positive.
    ...
    missing = [
        c for c in required_strength_cols
        if c not in trace.columns
    ]

    if missing:
        raise RuntimeError(
            f"Missing strength columns in {trace_path}: {missing}"
        )

    rigid = pd.to_numeric(
        trace["relative_rigid"],
        errors="coerce",
    ...
    rigid_thr = pd.to_numeric(
        trace["relative_rigid_thr"],
        errors="coerce",
    ).to_numpy(dtype=float)

    point_thr = pd.to_numeric(
        trace["relative_point_thr"],
        errors="coerce",
    ).to_numpy(dtype=float)
